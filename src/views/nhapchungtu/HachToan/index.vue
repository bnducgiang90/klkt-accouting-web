...
export default {
  ...
  methods: {
    ...mapActions("nhapchungtu", [
      "setRowFlag",
      "updateHachToanCell",
      "updateMultipleNhatKyChung",
      "loadTaiKhoanChiTiet",
    ]),
    handleRow(type, row) {
      console.log("handle", type, row);
      this.setRowFlag({
        stateName: "hachToanData",
        key: "dongChungTu",
        value: row.dongChungTu,
        flagName: type,
        row: row,
      });
    },
    async handlePressSpaceKey(data) {
      if (data.col.prop === "tkNo" || data.col.prop === "tkCo") {
        if (this.popupRef) {
          ...
          if (data.row[data.col.prop] === "131") {
          ...
          } else if (data.row[data.col.prop] === "331") {
            ...
            data.row[data.col.prop] === "141" ||
            data.row[data.col.prop] === "334"
          ) {
          ...
          } else if (data.row[data.col.prop] === "154") {
